...
.. php:class:: Table_Item
    ...
    .. php:method:: filterByTags($select, $tags)
        ...
        SELECT i.idFROM omeka_items iWHERE(i.id IN(SELECT tg.relation_id as idFROM omeka_taggings tgINNER JOIN omeka_tags t
        ON t.id = tg.tag_idWHERE t.name = 'foo' AND tg.type = 'Item')AND i.id IN(SELECT tg.relation_id as idFROM
        omeka_taggings tgINNER JOIN omeka_tags t ON t.id = tg.tag_idWHERE t.name = 'bar' AND tg.type = 'Item'))...
